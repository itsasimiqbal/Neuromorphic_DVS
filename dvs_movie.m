function [output] = dvs_movie(x, y, pol, ts, varargin)
% extension of events_film which is functional
...
% %frame representation  
 ...
 strcmpi(p.Results.mode, 'f') || strcmpi(p.Results.mode, 'frame')
    ...
    fprintf('\nsize\n');
    size(rot90(dvs_frames))
    aa1 = rot90(dvs_frames);
    aa2 = aa1 * 3;
    aa3 = aa2 * 3;
    size(aa3)
    aa_all(:,:,:,1) = [aa1];
    ...
    output = immovie(aa_all);
    implay(output); %if condition to choose the type of plot
